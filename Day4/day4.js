...
let input = `eyr:2029 pid:157374862
...
let invalidPasses = `eyr:1972 cid:100
...
let validPasss = `pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
iyr:2010 hgt:150cm hcl:#b6652a ecl:blu byr:1944 eyr:2020 pid:093154719

iyr:2010 hgt:150cm hcl:#b6652a ecl:amb byr:1944 eyr:2030 pid:009315471`
...
function main() {
    ...
    // console.log("Part 1")
    // console.log("test: ")
    // console.log(computePart1(test))
    // console.log("Expecting 2")
    // console.log("\nInput:")
    // console.log(computePart1(input))


    console.log("Part 2")
    console.log("test: ")
    console.log(computePart2(invalidPasses))
    console.log("Expecting 0")
    console.log(computePart2(validPasss))
    console.log("Expecting 4")
    console.log("\nInput:")
    console.log(computePart2(input))
}
...
function parseInput(input) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i] === '') {
            passportStrings.push(passportString.trim())
            passportString = ''
        } else {
            passportString += lines[i] + ' '
        }
    }
    passportStrings.push(passportString)

    return passportStrings.map(passport => passport.split(' ').filter(entry => entry !== "").map(entry => entry.split(':'))).map(passport => {
        let res = {}
        passport.forEach(entry => res[entry[0]] = entry[1])
        return res
    });
}

function computePart1(input) {
    let map = parseInput(input);


    function validatePart1(passport, key) {
        return passport.hasOwnProperty(key);
    }

    return map.filter(passport => {
        let booleans = requiredKeys.flatMap(key => validatePart1(passport, key));
        let containsFalse = booleans.reduce((acc, curr) => acc && curr);
...
function computePart2(input) {
    ...
    function validatePart2(passport, key) {
        if(passport.hasOwnProperty(key)){
            ...
            try {
                ...
                if (key === "hgt") {
                    ...
                    let hgt = parseInt(value.slice(0, -2))
                    return (unit === "cm" && (hgt >= 150 && hgt <= 193)) ||(unit === "in" && (hgt >= 59 && hgt <= 76))
                }
                if (key === "hcl") {
    ...
    return map.filter(passport => {
        let booleans = requiredKeys.flatMap(key =>
            validatePart2(passport, key)
        );
        let containsFalse = booleans.reduce((acc, curr) => acc && curr);
        return containsFalse
    }).length
